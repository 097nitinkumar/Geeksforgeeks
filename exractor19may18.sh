...
get_linecount()
{
	temp_lines=$( cat list.actual | wc -l )
...
moved_domain()
{
	...
	curl -i "$rname" -L | egrep -A 10 '301 Moved Permanently|302 Found' | grep 'Location' | awk -F': ' '{print $2}' | tail -1 >moved.txt
...
get_file_for_link()
{
	...
	if [ "$n_domain" == "" ]
		then
			...
			#so new url i.e. rname will be n_domain
...
{	
	...
	grep -Po '(?<=href=")[^"]*' file >> output_temp.log
	awk '!a[$0]++' output_temp.log>list.actual
...
storing_to_list()
{
	...
	while [ "$b" -lt "$temp_lines" ]    # run loop till no of lines in temp
	do
		#read line and store them in variable line
   		line=$(head -"$b" list.actual | tail -1)
   		
   		if [[ $line == /* ]]; 
			then 
				#foo=$rname$line
				#procedure_called $foo
				echo 'invalid link'
				((b++))
			else 
   				procedure_called $line
   				((b++))
			fi
